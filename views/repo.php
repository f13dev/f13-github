...
class Repo
{
    ...
    {
        $v = '<div class="f13-gist-container" style="border-radius: 10px 10px 0 0; border: 1px solid #45484d;">';
                ...
                if (is_array($this->files)) {
                    ...
                    foreach ($this->files as &$eachFile)
                    {
                        ...
                        if ($this->show_files != 'names') {
                            $v .= '<pre class="prettyprint lang-' . strtolower( $this->data['language'] ) . '" style="border: 1px solid black; margin: 10px 0; width: 100%; padding: 10px; '.(($this->show_files == 'full') ? 'overflow: scroll;' : 'overflow-x: scroll;max-height: 200px;').'">';
